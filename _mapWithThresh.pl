...
for my $thresh (@threshs){
    ...
    my $outputName = "./d/map_" . dtStr() . "_" . $thresh;
	...
	for my $weight (@fileNames){
		...
		my $cmd = "./darknet detector map d/obj.data d/yolo-obj.cfg d/weights/" . $weight .  " -thresh ". $thresh." -iou_thresh ".$thresh . " >>" . $outputName . ".txt";	
		my $echo = system($cmd);
	}
