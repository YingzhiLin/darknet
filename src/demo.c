...
void demo(char *cfgfile, char *weightfile, float thresh, float hier_thresh, int cam_index, const char *filename, char **names, int classes,
    ...
    while(1){
        ++count;
        {
            if(pthread_create(&fetch_thread, NULL, fetch_in_thread, 0)) error("Thread creation failed");
            if(pthread_create(&detect_thread, NULL, detect_in_thread, 0)) error("Thread creation failed");
...
void demo2(char* cfgfile, char* weightfile, float thresh, float hier_thresh, int cam_index, const char* filename, char** names, int classes,
    ...
    mat_cv* ori_img; // in video analysis, save the origin image.
    ori_img = NULL;
    ...
    fetch_in_thread(0); // img 存入in_img
    det_img = in_img;   // 这应该是共用存储空间的
    det_s = in_s;

    fetch_in_thread(0); // 继续取图
    detect_in_thread(0); // 读取的是det_img的内容
    det_img = in_img;
    det_s = in_s;
    ...
    while (1) {
        ++count;
        {
            ...
            if (prefix) {
                char buff[256];
                sprintf(buff, "%s_ORI_%08d.jpg", prefix, count);
                if (show_img) save_cv_jpg(show_img, buff);
            }
            ...
            /*
               下面这个有问题，第一轮的时候show_img尚未定义，因此第一张图就不见了。
               接着使用show_img的时候，是刚刚得到的图，但却不是正在处理的图
               因此，错位是不可避免的。
            */
            draw_detections_cv_v3(show_img, local_dets, local_nboxes, demo_thresh, demo_names, demo_alphabet, demo_classes, demo_ext_output);
            free_detections(local_dets, local_nboxes);            
